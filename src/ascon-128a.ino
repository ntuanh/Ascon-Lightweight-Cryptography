...
void loop() {
  ...
  ascon128a_encrypt(C, T, P, plen, A, 0, N, K);

  Serial.print("Ciphertext: ");
  for (int i = 0; i < plen; i++) Serial.printf("%02X ", C[i]);
  Serial.println();

  Serial.print("Tag: ");
  for (int i = 0; i < 16; i++) Serial.printf("%02X ", T[i]);
  Serial.println();

  u8 Pdec[128];
  if (ascon128a_decrypt(Pdec, C, plen, A, 0, N, K, T)) {
    Serial.print("Decrypted: ");
    for (int i = 0; i < plen; i++) Serial.print((char)Pdec[i]);
    Serial.println();
  } else {
